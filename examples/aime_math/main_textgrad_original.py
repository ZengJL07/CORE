...
def main() -> None:
    ...
    result = runner.run()
    print(
        f"[AIME] Original TextGrad optimization finished with final val score "
    ...
    experiment.report_final_results(
        result.final_prompt,
        label=f"original_{algorithm_config.algorithm}",
        output_stem=f"textgrad_original_{algorithm_config.algorithm}_result_plot",
    )
